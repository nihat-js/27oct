...
<body>
          ...
          <tr>
            ...
            <td class="actions">
              <button onclick="handleDelete(<?= $employee->id ?>)">
                <svg width="24px" width="24px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
  ...
  <script>
    ...
    async function handleSearchKeywords() {
      ...
      try {
        ...
        data.forEach((item, i) => {
          ...
          let str = `${item.first_name} ${item.last_name}`
          p.textContent = str
          p.onclick = () => {
            suggestions.innerHTML = ""
            form.fullName.value = str
          }
    ...
    async function handleDelete(id) {
      ...
      let response = await fetch(base_url + "employee/delete/"+id,{
        method : "POST"
      })
